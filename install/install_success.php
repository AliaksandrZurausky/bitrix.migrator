...
<div class="migrator-install-page">
    <?php if ($exception): ?>
        <div class="migrator-alert migrator-alert--error">
            ...
            <div>
                <strong><?= Loc::getMessage('BITRIX_MIGRATOR_INSTALL_FAILED') ?></strong>
    ...
    <?php else: ?>
        <div class="migrator-alert migrator-alert--success">
            <div class="migrator-alert-icon">&#10004;</div>
            <div>
                <strong><?= Loc::getMessage('BITRIX_MIGRATOR_INSTALL_SUCCESS') ?></strong>
                <p>Модуль готов к работе. Перейдите на страницу модуля для настройки подключения.</p>
            </div>
        </div>
    <?php endif; ?>

    <div style="display:flex;gap:10px;margin-top:20px;">
        <a href="/bitrix/admin/bitrix_migrator.php" class="migrator-btn migrator-btn--primary">
            Перейти к настройке
        </a>
...
.migrator-btn--primary { background: #0080C8; color: #fff; }
.migrator-btn--primary:hover { background: #006ba8; color: #fff; }
